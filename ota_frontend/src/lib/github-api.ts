...
export const fetchGitHubFile = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`GitHub API error (${response.status}): File not found at ${path}, might be a submodule`);
        ...
        if (commitsResponse.ok) {
          ...
          if (commits.length > 0) {
            ...
            if (treeResponse.ok) {
              ...
              const fileInTree = treeData.tree.find((item: any) => item.path === path);
              ...
              if (fileInTree && fileInTree.type === "blob") {
                ...
                if (blobResponse.ok) {
                  ...
                  if (blobData.content && blobData.encoding === "base64") {
                    const bytes = Uint8Array.from(atob(blobData.content), (c) => c.charCodeAt(0));
